...
const parseInput = (input) => {
  ...
  for (let row = 0; row < rows.length; row += 1) {
    for (let col = 0; col < rows[row].length; col += 1) {
      if (rows[row][col] === '#') {
        result[getKey(hyper, level, row, col)] = true;
      }
    }
  }
...
const getKey = (hyper, level, row, col) => {
  return `${hyper},${level},${row},${col}`;
};
...
const getNeighbors = (cube) => {
  const result = [];
  const [hyper, level, row, col] = cube
    .split(',')
    .map((value) => parseInt(value, 10));

  for (let h = hyper - 1; h <= hyper + 1; h += 1) {
    for (let l = level - 1; l <= level + 1; l += 1) {
      for (let r = row - 1; r <= row + 1; r += 1) {
        for (let c = col - 1; c <= col + 1; c += 1) {
          if (h === hyper && l === level && r === row && c === col) {
            continue;
          }

          result.push(getKey(h, l, r, c));
        }
      }
    }
  }
